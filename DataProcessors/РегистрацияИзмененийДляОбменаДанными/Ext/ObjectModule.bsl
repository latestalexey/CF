...
// Устанавливаем флаг поддержки БСП
//
...
//
Функция БСП_ДоступнаТребуемаяВерсия() Экспорт
	
	// Ищем БСП 2.1.2 и старше
	Попытка
		ЧастиВерсии = Вычислить("СтандартныеПодсистемыСервер.ВерсияБиблиотеки()");
	Исключение
		// Отсутствует или поломан метод определения версии, считаем БСП недоступной
		Возврат Ложь;
	КонецПопытки;
	
	ЧастиВерсии = СтрЗаменить(ЧастиВерсии, ".", Символы.ПС);
	...
	Для Индекс=0 По НужнаяВерсия.ВГраница() Цикл
		ЧастьТекущейВерсии = СтрокаВЧисло(СтрПолучитьСтроку(ЧастиВерсии, Индекс + 1));
		ЧастьНужнойВерсии  = НужнаяВерсия[Индекс];
