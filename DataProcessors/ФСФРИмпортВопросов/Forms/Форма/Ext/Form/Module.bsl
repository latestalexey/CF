...
Процедура СоздатьFB2(Команда)
	Т = СоздатьFB2_Сервер(СПравильнымиОтветами);
	Т.Записать("d:\fsfr\fb2\ФСФРБазовый.fb2");
КонецПроцедуры
 
&НаСервере
Функция СоздатьFB2_Сервер(СПравильнымиОтветами)
	...
	//создаем кэш правильных ответов
	Если СПравильнымиОтветами Тогда
		Запрос = Новый Запрос;
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	Рег.НомерОтвета,
		...
		|	Рег.НомерВопроса
		|ИЗ
		|	РегистрСведений.ФСФРБазовыйПравильныеОтветы КАК Рег";
		
		КэшОтветов = Запрос.Выполнить().Выгрузить();
		КэшОтветов.Индексы.Добавить("Глава, НомерВопроса");
	КонецЕсли;
	...
	Пока ВыборкаГлав.Следующий() Цикл
		...
		//для поиска правильного ответа в кэше
		СтруктураПоиска = Новый Структура("Глава", ВыборкаГлав.Глава);
		...
		Пока ВыборкаВопросов.Следующий() Цикл
			...
			//найдем правильный ответ в кэше
			Если СПравильнымиОтветами Тогда
				СтруктураПоиска.Вставить("НомерВопроса",ВыборкаВопросов.НомерВопроса);
				ПоискОтвета = КэшОтветов.НайтиСтроки(СтруктураПоиска);
				ПравильныйОтвет = Неопределено;
				Если ПоискОтвета.Количество()>0 Тогда
					ПравильныйОтвет = ПоискОтвета[0].НомерОтвета;
				КонецЕсли;
			КонецЕсли;
			...
			Пока ВыборкаОтветы.Следующий() Цикл
				Если СПравильнымиОтветами И ВыборкаОтветы.НомерВарианта = ПравильныйОтвет Тогда
					Т.ДобавитьСтроку("<p><strong>"+ВыборкаОтветы.НомерВарианта+"."+Символы.Таб+ВыборкаОтветы.ВариантОтвета+"</strong></p>");
				Иначе
...
&НаСервере
Процедура ДобавитьКартинкуНаСервере(КодВопроса)
	...
	МенеджерЗаписи = РегистрыСведений.ФСФРБазовыйКартинки.СоздатьМенеджерЗаписи();
	МенеджерЗаписи.КодВопроса = КодВопроса;
	МенеджерЗаписи.Глава	= Глава;
	МенеджерЗаписи.НомерВопроса = НомерВопроса;
	...
	МенеджерЗаписи.НомерВопроса = НомерВопроса;
		...
		МенеджерЗаписи.Картинка = Новый ХранилищеЗначения(Base64картинка);
		МенеджерЗаписи.ИмяКартинки = "#"+СтрЗаменить(КодВопроса, ".", "_")+".jpg";
...
Процедура ДобавитьКартинку(Команда)
	...
	КодВопроса = "";
	Если ВвестиСтроку(КодВопроса) = Ложь Тогда
		Возврат;
	КонецЕсли;
		
	ДобавитьКартинкуНаСервере(КодВопроса);
