...
//
&НаСервере
Процедура ОбработатьОбъект(МассивОбъектов)

	КоллекцияПечатныхФорм = ПолучитьКоллекцияПечатныхФорм();
	//
	//
	МенеджерПечати = ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени(ОбъектПоиска.Тип+"."+ОбъектПоиска.Имя);
	МенеджерПечати.Печать(МассивОбъектов, Новый Структура(), КоллекцияПечатныхФорм, Новый СписокЗначений, Новый Структура("ДоступнаПечатьПоКомплектно",Истина)); 
	//МенеджерПечати
...
&НаСервере
Функция ПолучитьКоллекцияПечатныхФорм()
	
	ИмяПФ = ПолучитьИмяПечатнойФормы();
	
	Для Каждого СтрокаПечФ из таблицаПечатныхФорм Цикл
		Если СтрокаПечФ.Использовать Тогда
			ИмяПФ = СтрокаПечФ.ИмяПечатнойФормы;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	...
	НоваяСтрокаКоллекции.ИмяМакета 		= ИмяПФ;
	НоваяСтрокаКоллекции.ИмяВРЕГ 		= ВРег(ИмяПФ);
...
&НаСервере
Функция ПолучитьИмяПечатнойФормы()
...
Функция ВыполнитьОбработку() Экспорт
	...
	ИмяПечатнойФормы = ПолучитьИмяПечатнойФормы();
	
	//МассивОбъектов = НайденныеОбъекты.ВыгрузитьЗначения();
	//ОбработатьОбъект(МассивОбъектов);		
	...
	Если ИмяПечатнойФормы <> "" Тогда
		...
		Для Индекс = 0 По НайденныеОбъекты.Количество() - 1 Цикл
			...
			МассивСсылки.Добавить(Объект);
			Если РежимПечати = 1 Тогда
				УправлениеПечатьюКлиент.ВыполнитьКомандуПечати(ОбъектПоиска.Тип+"."+ОбъектПоиска.Имя, ИмяПечатнойФормы, МассивСсылки, ЭтаФорма);
			Иначе
				УправлениеПечатьюКлиент.ВыполнитьКомандуПечатиНаПринтер(ОбъектПоиска.Тип+"."+ОбъектПоиска.Имя, ИмяПечатнойФормы, МассивСсылки);
			КонецЕсли;
		КонецЦикла;
		...
		Возврат Индекс;
	Иначе
		Сообщить("Не выбрана печатная форма!");
	КонецЕсли;
...
&НаСервере
Процедура УправлениеПечатью_ПриСозданииНаСервере(Форма, МестоРазмещенияКомандПоУмолчанию = Неопределено) Экспорт
	...
	Для Каждого МестоРазмещения Из МестаРазмещения Цикл
		...
		Если НайденныеКоманды.Количество() > 0 Тогда
			Для Каждого СтрокаКоманды из НайденныеКоманды Цикл
				НоваяПечФорма = таблицаПечатныхФорм.Добавить();
				НоваяПечФорма.ИмяПечатнойФормы = СтрокаКоманды.Идентификатор;
			КонецЦикла;		
		КонецЕсли;
	КонецЦикла;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// СтандартныеПодсистемы.Печать
		УправлениеПечатью_ПриСозданииНаСервере(ОбъектПоиска.Тип+"."+ОбъектПоиска.Имя+".Форма.ФормаДокумента", Элементы.таблицаПечатныхФорм);
	КонецЕсли;
