...
Функция СформироватьУсловиеПоискаПоСтроке()
	...
	Если УсловиеПоискаПоСтроке <> "" Тогда
		УсловиеПоискаПоСтроке = "("+УсловиеПоискаПоСтроке+")";	
	КонецЕсли;
	...
	Иначе
		...
		Если ОтборДанных <> Неопределено Тогда
			Для Каждого ЭлементОтбора Из ОтборДанных.Отбор.Элементы Цикл
				...
				|	И ") + ЭтотОбъект.ПолучитьВидСравнения(ЭлементОтбора.ЛевоеЗначение, ЭлементОтбора.ВидСравнения, ИмяПараметра);
...
Функция ПолучитьПолноеИмяФормы(ИмяНужнойФормы)
	...
	МассивСтрок = ЭтотОбъект.РазложитьСтрокуВМассивПодстрок(ЭтаФорма.ИмяФормы, ".");
	МассивСтрок[МассивСтрок.Количество() - 1] = ИмяНужнойФормы;
	
	Возврат ЭтотОбъект.ПолучитьСтрокуИзМассиваПодстрок(МассивСтрок, ".");
КонецФункции
